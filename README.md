...
* **Gelişmiş Yapay Zeka Entegrasyonu:** Google Gemini AI modeli ile doğal dil işleme (NLP) yetenekleri ve dinamik sohbet akışı.
* **Asenkron Veri Yönetimi:** API isteklerinin ve veri çekme durumlarının (loading, error, success) TanStack Query kullanılarak optimize edilmesi.
* **Kullanıcı Deneyimi (UX):** Mesaj yazılıyor (Typing Indicator) animasyonları, otomatik kaydırma (auto-scroll) ve temiz, modern bir mesajlaşma arayüzü (MessageList & MessageInput).
